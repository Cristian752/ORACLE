-- a)a)	Proyecte el id del departamento con la cantidad de empleados que hay en él
...
-- f)	Seleccione los empleados que ingresaron a trabajar en el mismo mes del mes actual y proyectar un saludo que diga: 
   --  “Estimado NOMBRE, es para nosotros un gusto que hayas compartido con nosotros durante los últimos X días. 
   --   Queremos expresarte que puedes contar con nosotros y que contamos contigo. Por favor pasa el ULTIMO_DIA_DEL MES por el salón principal para la reunión de celebración”.
...
-- g)	Proyecta el ID, nombre, apellido, salario y experiencia. La experiencia se determina en 1, 2, 3 o 4 así:
...
-- j) que hace el query?
-- Se hace una consulta para la sumatoria de salarios por meses 
-- y nivel de departamento creando una tabla donde muestra la tabla

-- k) que hace el query?
-- Hace un registro para cada apellido y listado de apellidos separados por ; con la misma fecha de ingreso
